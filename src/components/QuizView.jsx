...
export function QuizView({ active, page, setBanner }) {
  ...
  async function generateBatch(currentQuiz = quiz) {
  ...
  function recordCoverage(current, question, correct) {
  ...
  function restartQuiz() {
    updateQuiz((current) => createInitialQuiz(current.difficulty));
  }

  function selectOption(index, checked, multiple) {
    updateQuiz((current) => ({
      ...current,
      selected: multiple
        ? checked
